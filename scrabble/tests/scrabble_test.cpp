...
TEST_F(DictionaryTest, is_word0) { EXPECT_TRUE(d.is_word("hi")); }

TEST_F(DictionaryTest, is_word1) { EXPECT_TRUE(d.is_word("hello")); }

TEST_F(DictionaryTest, is_word2) { EXPECT_FALSE(d.is_word("hel")); }

TEST_F(DictionaryTest, is_word3) { EXPECT_TRUE(d.is_word("abstractionists")); }

TEST_F(DictionaryTest, is_word4) { EXPECT_TRUE(d.is_word("abstractionist")); }

TEST_F(DictionaryTest, is_word5) { EXPECT_TRUE(d.is_word("abstract")); }

TEST_F(DictionaryTest, is_word6) { EXPECT_FALSE(d.is_word("abstractio")); }

TEST_F(DictionaryTest, is_word7) { EXPECT_FALSE(d.is_word("abstr")); }

TEST_F(DictionaryTest, is_word8) { EXPECT_TRUE(d.is_word("a")); }
...
bool AnchorTest::anchor_lookup(const vector<Board::Anchor> a, Board::Anchor anchor) {
    ...
    for (size_t i = 0; (i < a.size() && !found); ++i) {
        if (a[i].position == anchor.position && a[i].direction == anchor.direction && a[i].limit == anchor.limit)
            found = true;
    }
    return found;
}
