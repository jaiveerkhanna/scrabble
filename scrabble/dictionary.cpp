...
bool Dictionary::is_word(const string& word) const {
    ...
    // if is_final is true, the word is a standalone word and we should return true
    if (cur->is_final == true) {
        return true;
    }
    // If it is not final, it is not a standalone word
    else {
        return false;
    }
}
...
vector<char> Dictionary::next_letters(const std::string& prefix) const {
    ...
    // use an iterator to go through map nexts and add the key value to nexts
    map<char, std::shared_ptr<TrieNode>>::iterator it = cur->nexts.begin();
